...
function Watch(props) {
    ...
    const formatTime = (date) => {
        let hours = date.getUTCHours(), minutes = date.getUTCMinutes(), seconds = date.getUTCSeconds();
        const offset = hours + Number(timezone);
        if (offset >= 24) hours = offset - 24;
        else if (offset < 0) hours = offset + 24;
        else hours = offset;
    ...
    return (
        <div>
            <div className="time-header-wrapper">
                ...
                <button className="delete-button" onClick={handleDelete}>х</button></div>
